...
private:
	...
	/* --Add-- */
	void AddFunInfoToLbl(EventLabel *lab);
	void tryFindComSoPath(WriteLabel *lab, std::vector<FunctionEvent> *com, std::vector<FunctionEvent> *so);
	std::vector<FunctionEvent> calcBasicCOM(Event e) const;
	std::vector<FunctionEvent> calcBasicSO(Event e) const;
	// --end
};
