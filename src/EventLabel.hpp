...
public:
	...
	enum EventLabelKind {
		...
		EL_RCULockLKMM,
		EL_RCUUnlockLKMM,
		// --For adding the concurrent-datastructure models--
		EL_FunStart,
		EL_FunEnd,
		// --end--
	};
...
public:
	...
	/* --Add-- */
	const FunctionName& getMethod() const { return method; }
	void setMethod(FunctionName s) { method = s; }

	unsigned int getInvocation() const { return invocation; }
	void setInvocation(unsigned int s) { invocation = s; }

	const std::vector<FunctionEvent>& getSO() const { return SO; }
	const std::vector<FunctionEvent>& getCOM() const { return COM; }

	void setSO(std::vector<FunctionEvent> &&so) { SO = std::move(so); }
	void setCOM(std::vector<FunctionEvent> &&com) { COM = std::move(com); }
	// --end--
...
private:
	...
	std::vector<FunctionEvent> SO;
	std::vector<FunctionEvent> COM;
	// --end--
...
/* Label for function calls, used to track the method that operations belong to */

class FunStartLabel : public EventLabel {

protected:
	FunStartLabel(EventLabelKind k, unsigned int st, llvm::AtomicOrdering ord,
		       Event pos, std::vector<SAddr> arguments)
		: EventLabel(k, st, ord, pos), args(arguments) {}

public:
	FunStartLabel(Event pos)
		: EventLabel(EL_FunStart, llvm::AtomicOrdering::NotAtomic, pos) {}
	template<typename... Ts>
	static std::unique_ptr<FunStartLabel> create(Ts&&... params) {
		return std::make_unique<FunStartLabel>(std::forward<Ts>(params)...);
	}

	std::unique_ptr<EventLabel> clone() const override {
		return std::make_unique<FunStartLabel>(*this);
	}

	static bool classof(const EventLabel *lab) { return classofKind(lab->getKind()); }
	static bool classofKind(EventLabelKind k) { return k == EL_FunStart; }

	std::vector<SAddr> getArgs() const { return args; }

private:

	std::vector<SAddr> args;
};
 ...
 ******************************************************************************/
 ...
 /* Label for function returns, used to track the method that operations belong to */

class FunEndLabel : public EventLabel {

public:
	FunEndLabel(Event pos)
		: EventLabel(EL_FunEnd, llvm::AtomicOrdering::NotAtomic, pos) {}
	template<typename... Ts>
	static std::unique_ptr<FunEndLabel> create(Ts&&... params) {
		return std::make_unique<FunEndLabel>(std::forward<Ts>(params)...);
	}

	std::unique_ptr<EventLabel> clone() const override {
		return std::make_unique<FunEndLabel>(*this);
	}

	static bool classof(const EventLabel *lab) { return classofKind(lab->getKind()); }
	static bool classofKind(EventLabelKind k) { return k == EL_FunEnd; }
};
// --end--
