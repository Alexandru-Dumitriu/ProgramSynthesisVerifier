...
public:
	...
	/* --Add-- */
	/* Adds the new labels to the graph */
	FunctionName checkFunctionName(std::string functionName);
	void visitFunctionStart(std::unique_ptr<FunStartLabel> lab, const EventDeps *deps, FunctionName name);
	void visitFunctionEnd(std::unique_ptr<FunEndLabel> lab, const EventDeps *deps);
	// --end
...
protected:
	...
	/*** Concurrent datastructure model checking related: Casper Henkes ***/
	/* --Add-- */

	/* The consistency check. */
	void checkModelViolations(Event pos);
	void visitModelViolation(Event pos, std::string err, std::vector<FunctionCall> funcalls, FunctionEvent error,
						std::map<OrderType, std::map<FunctionEvent, std::vector<FunctionEvent>>> rel);
	void checkMutex(Event pos);
	void checkQueue(Event pos);
	void checkStack(Event pos);
	void checkHp(Event pos);
	std::string doGetVarName(const SAddr &addr) const {
		return getVarName(addr);
	}
	std::vector<FunctionCall> findallFunctionCalls();
	std::map<OrderType, std::map<FunctionEvent, std::vector<FunctionEvent>>> calculateOrder(std::vector<FunctionCall> functions);
	// --end
...
private:
	...
	/* --Add-- */
	/* Outputs the current functioncall graph into a file (DOT format) */
	void dotPrintCallGraphToFile(const std::string &filename, std::vector<FunctionCall> functions, FunctionEvent error, 
						std::map<OrderType, std::map<FunctionEvent, std::vector<FunctionEvent>>> rel);
	/* --end */
