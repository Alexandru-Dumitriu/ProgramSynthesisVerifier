...
static llvm::cl::opt<std::string>
clModelConfigFile("model-file", llvm::cl::init(""), llvm::cl::value_desc("file"),
	       llvm::cl::cat(clGeneral),
	       llvm::cl::desc("A file that describes the model to be checked"));
/* --end */
...
void Config::saveConfigOptions()
{
	...
	/* --add-- */
	/* Save model checking options */
	datastructureModel = loadDSModelFromFile(clModelConfigFile);
	/* --end */
#ifdef ENABLE_GENMC_DEBUG
