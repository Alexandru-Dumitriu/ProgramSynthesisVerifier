...
public:
	...
	/* --Add-- */
	FunctionName previousEventMethod(Event e);

	unsigned int getFunInvocationCount(FunctionName funName);
	unsigned int previousEventInvocation(Event e);
	const EventLabel *getLastFunEvent(FunctionName funName, unsigned int invocation);
	/* --End-- */
