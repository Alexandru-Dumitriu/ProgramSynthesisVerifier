...
/* Calulates PO, COM, invCOM, SO, and LHB for all functioncalls. */
std::map<OrderType, std::map<FunctionEvent, std::vector<FunctionEvent>>> GenMCDriver::calculateOrder(std::vector<FunctionCall> functions) 
{
	std::map<OrderType, std::map<FunctionEvent, std::vector<FunctionEvent>>> relations;
	std::vector<std::pair<FunctionEvent, FunctionEvent>> povec, comvec, sovec, lhbvec;
	for(auto f : functions) {
		// Go through all functions to find PO and LHB
		int idx = std::find(functions.begin(), functions.end(), f) - functions.begin();

		// PO
		// Because of how functioncalls are ordered direct PO can only be with the next functioncall.
		if(idx < functions.size() - 1) {
			FunctionCall testf = functions[idx+1];
			if(f.getlabels()[0]->getThread() == testf.getlabels()[0]->getThread()) {
				povec.push_back(std::pair<FunctionEvent, FunctionEvent>(f.getEvent(), testf.getEvent()));
			}
		}
		// Add in COM.
		for(auto c : f.getCOMorder()) {
			// COM here is from deq -> enq and should be other way around.
			comvec.push_back(std::pair<FunctionEvent, FunctionEvent>(c, f.getEvent()));
		}
		// Add in SO.
		for(auto s : f.getSOorder()) {
			sovec.push_back(std::pair<FunctionEvent, FunctionEvent>(s, f.getEvent()));
		}

		// LHB vec
		// Could add stricter LHB conditions here.
	}

	// Add the relations to a map for easy access and calculate the inverse of COM and SO.
	std::map<FunctionEvent, std::vector<FunctionEvent>> po, com, invcom, so, invso, lhb;
	for (auto f : functions) {
		std::vector<FunctionEvent> vpo, vcom, vinvcom, vso, vinvso, vlbh;
		for (auto rpo : povec) {
			if(rpo.first == f.getEvent()) { vpo.push_back(rpo.second); }
		}
		for (auto rcom : comvec) {
			if(rcom.first == f.getEvent()) { vcom.push_back(rcom.second); }
			if(rcom.second == f.getEvent()) { vinvcom.push_back(rcom.first); }
		}
		for (auto rso : sovec) {
			if(rso.first == f.getEvent()) { vso.push_back(rso.second); }
			if(rso.second == f.getEvent()) { vinvso.push_back(rso.first); }
		}
		po[f.getEvent()] = vpo;
		com[f.getEvent()] = vcom;
		invcom[f.getEvent()] = vinvcom;
		so[f.getEvent()] = vso;
		invso[f.getEvent()] = vinvso;
	}

	// Fix LHB (without this not correctly transitive)
	// This uses PO and SO to determine LHB so needs to be calculated after.
	for (auto testf : functions) {
		std::vector<FunctionEvent> poso;
		for (auto v : lhbvec) {
			if(v.first == testf.getEvent()) {
				poso.push_back(v.second);
			}
		}
		for (auto v : po[testf.getEvent()]) {
			if(std::find(poso.begin(), poso.end(), v) == poso.end()) {
				poso.push_back(v);
			}
		}	
		for (auto v : so[testf.getEvent()]) {
			if(std::find(poso.begin(), poso.end(), v) == poso.end()) {
				poso.push_back(v);
			}
		}
		...
		lhb[testf.getEvent()] = poso;
	}

	// adding all calculated orders to the map and returning them.
	relations[OrderType::PO] = po;
	relations[OrderType::COM] = com;
	relations[OrderType::INVCOM] = invcom;
	relations[OrderType::SO] = so;
	relations[OrderType::INVSO] = invso;
	relations[OrderType::LHB] = lhb;
	return relations;
}

/* Searches the entire execution graph to find all existing functioncalls. */
std::vector<FunctionCall> GenMCDriver::findallFunctionCalls() {
	auto &g = getGraph();

	std::vector<FunctionCall> functioncalls;

	// Finds all events that are endpoints of COM edges.
	std::vector<Event> COMVertices;
	for(int i = 0; i < g.getNumThreads(); i++) {
		for(int j = 0; j < g.getThreadSize(i); j++) {
			if(i == 0 && j < 2) {
				continue;
			}
			auto *lab = g.getEventLabel(Event(i, j));
			auto *prev = g.getEventLabel(Event(i, j-1));
			if(auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
				if(rLab->getCOM().size() > prev->getCOM().size()) {
					COMVertices.push_back(rLab->getPos());
					COMVertices.push_back(rLab->getRf());
				}
			} else if (auto *cwLab = llvm::dyn_cast<CasWriteLabel>(lab)) {
				if(cwLab->getCOM().size() > prev->getCOM().size()) {
					// if this is part of COM and a CASWriteLabel then the previous is read.
					if(auto *rLab = llvm::dyn_cast<ReadLabel>(prev)) {
						COMVertices.push_back(rLab->getPos());
						COMVertices.push_back(rLab->getRf());
					}
				}
	...
	// creates all functioncalls.
	for(int i = 0; i < g.getNumThreads(); i++) {
		FunctionName currFunc = FunctionName::FN_NON;
		std::vector<int> currvals;
		std::vector<EventLabel*> lbl;
		std::vector<WriteLabel*> w;
		std::vector<ReadLabel*> r;
		for(int j = 0; j < g.getThreadSize(i); j++) {
			if(i == 0 && j < 1) {
				continue;
			}
			auto *lab = g.getEventLabel(Event(i, j));
			// start of functioncall so reset everything.
			if(lab->getKind() == EventLabel::EL_FunStart) {
				currFunc = lab->getMethod();
			}
			if(lab->getMethod() == currFunc && currFunc != FunctionName::FN_NON) {
				lbl.push_back(lab);
				if(currFunc == FunctionName::FN_Protect0 || currFunc == FunctionName::FN_Unprotect0 || currFunc == FunctionName::FN_Protect1 || currFunc == FunctionName::FN_Unprotect1 || currFunc == FunctionName::FN_Reclaim) {
					currvals.push_back(i);
				}
				if(auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
					if (std::find(COMVertices.begin(), COMVertices.end(), Event(i, j)) != COMVertices.end()) {
						// This event is part of a COM edge.
						currvals.push_back(getReadValue(rLab).getSigned());
					}
					r.push_back(rLab);
				}
				if(auto *wLab = llvm::dyn_cast<WriteLabel>(lab)) {
					if (std::find(COMVertices.begin(), COMVertices.end(), Event(i, j)) != COMVertices.end()) {
						// This event is part of a COM edge.
						currvals.push_back(getWriteValue(wLab).getSigned());
					}
					w.push_back(wLab);
				}
				// still in function but the thing is done, so create the call but its not done yet.
				if(lab->getKind() == EventLabel::EL_FunEnd) {
					currFunc = FunctionName::FN_NON;
					FunctionCall A = FunctionCall(
						true, 
						FunctionEvent(lbl[0]->getMethod(), lbl[0]->getInvocation()), 
						std::vector<EventLabel*>(lbl), 
						std::vector<ReadLabel*>(r), 
						std::vector<WriteLabel*>(w),
						std::vector<int>(currvals)
					);
					functioncalls.push_back(A);
					currvals.clear();
					lbl.clear();
					w.clear();
					r.clear();
				} else if(lab->getKind() == EventLabel::EL_FunStart) {
					currFunc = FunctionName::FN_NON;
					FunctionCall A = FunctionCall(
						false, 
						FunctionEvent(lbl[0]->getMethod(), lbl[0]->getInvocation()), 
					...
					w.clear();
					r.clear();
				} else if(lab->getIndex() == g.getThreadSize(i)-1 && currFunc != FunctionName::FN_NON) {
					currFunc = FunctionName::FN_NON;
					FunctionCall A = FunctionCall(
						false, 
						FunctionEvent(lbl[0]->getMethod(), lbl[0]->getInvocation()), 
...
/* Helper function for trying to calculate the total order according to lhb. If the size is different from the input then it failed. */
/* Later items in this TO happen before earlier items. */
std::vector<FunctionEvent> tryCalcTO(std::map<FunctionEvent, std::vector<FunctionEvent>> lhb, std::vector<FunctionCall> functioncalls) {
	std::vector<FunctionEvent> to;
	std::vector<FunctionEvent> calls;
	for(auto fun : functioncalls) {
		calls.push_back(fun.getEvent());
	}
	while (calls.size() > 0) {
		FunctionEvent *current = nullptr;
		for (auto fun : calls) {
			bool check = true;
			if (to.size() > 0) {
				check = false;
				for (auto testlhb : lhb[fun]) {
					if (testlhb == to[to.size()-1]) { check = true; }
				}
			}
			for (auto event : lhb[fun]) {
				// if this item is the current last item then it should only happen before events already in the list.
				if (std::find(to.begin(), to.end(), event) == to.end()) {
					check = false;
				}
			}
			if (check) {
		...
		if (current == nullptr) {
			// cannot find further items to add to TO.
			return to;
		}
		// Add the found last item to TO.
		to.push_back(*current);
		// Remove the item from calls to not check it again.
		calls.erase(std::find(calls.begin(), calls.end(), *current));
	}
	return to;
}

/* Prints all the relations found in rel before going to the error routine. */
void GenMCDriver::visitModelViolation(Event pos, std::string err, std::vector<FunctionCall> funcalls, FunctionEvent error,
					std::map<OrderType, std::map<FunctionEvent, std::vector<FunctionEvent>>> rel) {
	// print interesting relations:
	llvm::outs() << "All found relations at the moment the error was detected.\n";
	for (auto tmap : rel) {
		llvm::outs() << tmap.first << ": " << "| ";
		for (auto v : tmap.second) {
			if (v.second.size() == 0) {
				continue;
			}
			llvm::outs() << v.first << " (";
			for (int i = 0; i < v.second.size(); i++) {
				llvm::outs() << v.second[i];
	...
	// prints the error in DOT format.
	dotPrintCallGraphToFile("tests/error.dot", funcalls, error, rel);
	// Reports the error and stops execution.
	visitError(pos, Status::VS_ModelViolation, err);
}

/* TODO rewrite to Axioms so you can reuse code. maybe put in seperate file / class?*/
// Axioms here.


void GenMCDriver::checkModelViolations(Event pos) {
	switch (getConf()->datastructureModel.type)
	{
	case DSModelType::M_None:
		break;
	case DSModelType::M_Mutex:
	//	llvm::outs() << " > check";
		checkMutex(pos);
		break;
	case DSModelType::M_Rel_Queue:
	case DSModelType::M_Weak_Queue:
	case DSModelType::M_HW_Queue:
	case DSModelType::M_Strong_Queue:
		checkQueue(pos);
		break;
	case DSModelType::M_Rel_Stack:
	case DSModelType::M_Weak_Stack:
	case DSModelType::M_C_Stack:
		checkStack(pos);
		break;
	case DSModelType::M_Hp_Queue:
		checkHp(pos);
		break;
	default:
		llvm::outs() << "Model not yet implemented for verifying!\n";
		BUG();
		break;
	}
}
...
/**************************************************************/
/* Checks 5 properties the Mutex execution should satisfy. */
/**************************************************************/
void GenMCDriver::checkMutex(Event pos) {
	auto &g = getGraph();
	auto *label = g.getEventLabel(pos);
	FunctionEvent currFunEvent = FunctionEvent(label->getMethod(), label->getInvocation());

	if(currFunEvent.name == FN_Init) {
		// should only happen at the start so should be fine. TODO test?
		return;
	}

	bool error = false;
	std::string str;
	llvm::raw_string_ostream errorMessage(str);
	errorMessage << "Mutex: Found one or multiple modelviolations\n";

	std::vector<FunctionCall> functioncalls = findallFunctionCalls();

	std::map<OrderType, std::map<FunctionEvent, std::vector<FunctionEvent>>> relations = calculateOrder(functioncalls);

	int coms = relations[COM][currFunEvent].size();
	int invcoms = relations[INVCOM][currFunEvent].size();
	switch (currFunEvent.name)
	{
	case FunctionName::FN_Lock:
		if (coms != 0) {
			/* 2: com relates matching unlock and lock events. */
			error = true;
			errorMessage << "2: COM should relate matching unlock and lock events but " << coms << " is not 0.\n";
		}
		if (invcoms != 1) {
			error = true;
			/* 3: Every lock is matched by at most one event and vice versa. */
			/* 4: All lock events are matched. */
			errorMessage << "3 & 4: All lock events should be matched by at most one event but " << invcoms << " is not 1.\n";
		}
		break;
	case FunctionName::FN_Unlock:
		if (invcoms != 0) {
			/* 2: com relates matching unlock and lock events. */
			error = true;
			errorMessage << "2: COM should relate matching unlock and lock events but " << invcoms << " is not 0.\n";
		}
		break;
	default:
		llvm::outs() << currFunEvent.name << "\n";
		BUG();
		break;
	}
	if(relations[OrderType::COM][currFunEvent] != relations[OrderType::SO][currFunEvent]) {
		/* 5: Every matching edge is synchronising: SO = COM. */
		error = true;
		errorMessage << "5: Every matching edge should be synchronizing (SO = COM).\n";
	}
	//if(functioncalls.size() > 3) {error = true;}
	if(error) { visitModelViolation(pos, errorMessage.str(), functioncalls, currFunEvent, relations); }
}

/**************************************************************/
/* Checks 7 properties the HW_Queue execution should satisfy. */
/**************************************************************/
void GenMCDriver::checkQueue(Event pos) {
	auto &g = getGraph();
	auto *label = g.getEventLabel(pos);
	FunctionEvent currFunEvent = FunctionEvent(label->getMethod(), label->getInvocation());
	int funValue;
	...
	std::string str;
	llvm::raw_string_ostream errorMessage(str);
	errorMessage << "Queue: Found one or multiple modelviolations\n";

	std::vector<FunctionCall> functioncalls = findallFunctionCalls();
	for(auto f : functioncalls) {
		if(f.getEvent() == currFunEvent) {
			if (f.isValueValid()) {
				funValue = f.getVal(0);
			//	llvm::outs() << currFunEvent << " with: " << funValue << "\n";
				valExists = true;
			}
		}
	}
	...
	case FunctionName::FN_Enq:
		if (relations[OrderType::COM][currFunEvent].size() > 1) {
			/* 3: Every enqueue is matched by at most one dequeue and vice versa. */
			error = true;
			errorMessage << "3: " << pos << " Missmatch in COM: should be one : one, deq : enq.\n";
		}
		if (relations[OrderType::INVCOM][currFunEvent].size() > 0) {
			/* 2: com relates matching enqueue and dequeue events. */
			error = true;
			errorMessage << "2: " << pos << " COM error: Enq should not be matched. INVCOM should be empty.\n";
		}
		break;
	case FunctionName::FN_Deq:
		if (relations[OrderType::COM][currFunEvent].size() > 0) {
			/* 2: com relates matching enqueue and dequeue events. */
			error = true;
			errorMessage << "2: " << pos << " COM error: Deq should not match. COM should be empty.\n";
		}
		if (relations[OrderType::INVCOM][currFunEvent].size() == 0 && label->getKind() == EventLabel::EL_FunEnd) {
			if (valExists) {
				/* 4: Every unmatched dequeue returns empty. */
				error = true;
				errorMessage << "4: " << pos << ": Unmatched deq returned non-empty value:" << funValue << "\n";
			}
			for (auto testfun : functioncalls) {
				if (testfun.getEvent().name == FN_Enq && relations[OrderType::COM][testfun.getEvent()].size() == 0 && testfun.isDone() && getConf()->datastructureModel.type != M_Weak_Queue) {
					std::vector<FunctionEvent> testLHB = relations[OrderType::LHB][testfun.getEvent()];
					// Need to find that the Enq lhb the Deq otherwise you are not sure that it actually happened before the other one.
					if(std::find(testLHB.begin(), testLHB.end(), currFunEvent) != testLHB.end()) {
						/* 5: dequeues with previous unmatched enqueues cannot return empty. */
						error = true;
						errorMessage << "5: " << pos << ": Dequeues with previous unmatched enqueue " << testfun.getEvent() << " cannot return empty.\n";
						break;
					}
				}
			}
	...
	if(getConf()->datastructureModel.type != M_Rel_Queue) {
		for (auto fun : functioncalls) {
			if (relations[OrderType::COM][fun.getEvent()].size() != relations[OrderType::SO][fun.getEvent()].size()) {
				/* 6: Every matching edge is synchronizing thus COM should be equal to SO. */
				error = true;
				errorMessage << "6: " << pos << " Every matching edge should be synchronizing (SO = COM).\n";
			}
		}
	}
	if (getConf()->datastructureModel.type != M_Strong_Queue) {
		for(auto fun : functioncalls) {
			FunctionEvent A = fun.getEvent();
			std::vector<FunctionEvent> currLHB = relations[OrderType::LHB][A];
			for (auto B : currLHB) {
				...
				if (A.name == B.name && A.name == FunctionName::FN_Deq && relations[OrderType::INVCOM][A].size() == 1 && relations[OrderType::INVCOM][B].size() == 1) {
					// C(enq) COM A(deq), D(enq) COM B(deq).
					FunctionEvent C = relations[OrderType::INVCOM][A][0]; FunctionEvent D = relations[OrderType::INVCOM][B][0];
					std::vector<FunctionEvent> DLHB = relations[OrderType::LHB][D];
					if (std::find(DLHB.begin(), DLHB.end(), C) != DLHB.end()) {
							error = true;
							errorMessage << "7: " << pos << " Not FiFo: " << A << LHB << B << ", " << D << LHB << C << ", " << C << COM << A << ", and " << D << COM << B << "\n";
					}
				}
				if (A.name == B.name && A.name == FunctionName::FN_Enq && relations[OrderType::COM][A].size() == 1 && relations[OrderType::COM][B].size() == 1) {
					// A(enq) COM C(deq), B(enq) COM D(deq).
					FunctionEvent C = relations[OrderType::COM][A][0]; FunctionEvent D = relations[OrderType::COM][B][0];
					std::vector<FunctionEvent> DLHB = relations[OrderType::LHB][D];
					if (std::find(DLHB.begin(), DLHB.end(), C) != DLHB.end()) {
							error = true;
							errorMessage << "7: " << pos << " Not FiFo: " << A << LHB << B << ", " << D << LHB << C << ", " << A << COM << C << ", and " << B << COM << D << "\n";
					}
				}
			}
		}
	} else {
		std::vector<FunctionEvent> to = tryCalcTO(relations[OrderType::LHB], functioncalls);
		if(to.size() != functioncalls.size()) {
			error = true;
			errorMessage << "7: " << pos << " Cannot create TO.\n";
			for (auto t : relations[LHB]) {
				for (auto e : t.second) {
					errorMessage << "|" << t.first << " " << LHB << " " <<  e << "| ";
				}
			}
			errorMessage << "\n";
		}
		// Now we have a TO, just need to check FiFo.
		for(int i = to.size(); i > 0; i--) {
			// the first event is last.
			for(int j = i-1; j >= 0; j--) {
				FunctionEvent A = to[i]; FunctionEvent B = to[j];
				if (A.name == FunctionName::FN_Enq && B.name == FunctionName::FN_Enq && relations[OrderType::COM][A].size() == 1 && relations[OrderType::COM][B].size() == 1) {
					// A(enq) COM C(deq), B(enq) COM D(deq). Higher position in TO means hb.
					FunctionEvent C = relations[OrderType::COM][A][0]; FunctionEvent D = relations[OrderType::COM][B][0];
					if (std::find(to.begin(), to.end(), C) < std::find(to.begin(), to.end(), D)) {
						error = true;
						errorMessage << "7: " << pos << " TO is not FiFo: " << A << LHB << B << ", " << D << LHB << C << ", " << A << COM << C << ", and " << B << COM << D << "\n";
					}
				}
				if (to[i].name == FunctionName::FN_Deq && to[j].name == FunctionName::FN_Deq && relations[OrderType::COM][to[i]].size() == 1 && relations[OrderType::COM][to[j]].size() == 1) {
					// C(enq) COM A(deq), D(enq) COM B(deq). Higher position in TO means hb.
					FunctionEvent C = relations[OrderType::INVCOM][A][0]; FunctionEvent D = relations[OrderType::INVCOM][B][0];
					if (std::find(to.begin(), to.end(), relations[OrderType::COM][to[i]][0]) < std::find(to.begin(), to.end(), relations[OrderType::COM][to[j]][0])) {
						error = true;
						errorMessage << "7: " << pos << " TO is not FiFo: " << A << LHB << B << ", " << D << LHB << C << ", " << C << COM << A << ", and " << D << COM << B << "\n";
					}
				}
			}
		}
	...
	// DEBUG
	//if (functioncalls.size() > 4) {error = true;}
	if(error) { visitModelViolation(pos, errorMessage.str(), functioncalls, currFunEvent, relations); }
}

/****************************************************************/
/* Checks 6 properties the weak-stack execution should satisfy. */
/****************************************************************/
void GenMCDriver::checkStack(Event pos) {
	auto &g = getGraph();
	auto *label = g.getEventLabel(pos);
	FunctionEvent currFunEvent = FunctionEvent(label->getMethod(), label->getInvocation());
	int funValue;
	...
	std::string str;
	llvm::raw_string_ostream errorMessage(str);
	errorMessage << "Stack: Found one or multiple modelviolations\n";
	...
	case FunctionName::FN_Push:
		if (relations[OrderType::COM][currFunEvent].size() > 1) {
			/* 3: Every push is matched by at most one pop and vice versa. */
			error = true;
			errorMessage << "Missmatch in COM: should be one : one, push : pop.\n";
		}
		if (relations[OrderType::INVCOM][currFunEvent].size() > 0) {
			/* 2: com relates matching push and pop events. */
			error = true;
			errorMessage << "COM error: push should not be matched. INVCOM should be empty.\n";
		}
		break;
	case FunctionName::FN_Pop:
		if (relations[OrderType::COM][currFunEvent].size() > 0) {
			/* 2: com relates matching push and pop events. */
			error = true;
			errorMessage << "COM error: pop should not match. COM should be empty.\n";
		}
		if (relations[OrderType::INVCOM][currFunEvent].size() == 0 && label->getKind() == EventLabel::EL_FunEnd) {
			if (valExists) {
				/* 4: Every unmatched pop returns empty. */
				error = true;
				errorMessage << "Unmatched pop returned non-empty value.\n";
			}
			for (auto testfun : functioncalls) {
				if (testfun.getEvent().name == FN_Push && relations[OrderType::COM][testfun.getEvent()].size() == 0 && testfun.isDone() && getConf()->datastructureModel.type != M_Weak_Stack) {
					std::vector<FunctionEvent> testLHB = relations[OrderType::LHB][testfun.getEvent()];
					// Need to find that the Push lhb the Pop otherwise you are not sure that it actually happened before the other one.
					if(std::find(testLHB.begin(), testLHB.end(), currFunEvent) != testLHB.end()) {
						/* 5: pops with previous unmatched pushes cannot return empty. */
						error = true;
						errorMessage << pos << ": Pop with previous unmatched Push " << testfun.getEvent() << " cannot return empty.\n";
						break;
					}
				}
			}
	...
	if(getConf()->datastructureModel.type != M_Rel_Stack) {
		for (auto fun : functioncalls) {
			if (relations[OrderType::COM][fun.getEvent()].size() != relations[OrderType::SO][fun.getEvent()].size()) {
				/* 5 or 6: Every matching edge is synchronizing thus COM should be equal to SO. */
				error = true;
				errorMessage << "6: " << pos << " Every matching edge should be synchronizing (SO = COM).\n";
			}
		}
	...
	for(auto fun : functioncalls) {
		...
		for (auto B : currLHB) {
			// FunctionEvent 'A' lhb 'B'. 
			// check if both events are of the same type. If so we need to be able to match the events that are com(-1) with them.
			if (A.name == B.name && A.name == FunctionName::FN_Pop && relations[OrderType::INVCOM][A].size() == 1 && relations[OrderType::INVCOM][B].size() == 1) {
				// C(push) COM A(pop), D(push) COM B(pop).
				FunctionEvent C = relations[OrderType::INVCOM][A][0]; FunctionEvent D = relations[OrderType::INVCOM][B][0];
				std::vector<FunctionEvent> CLHB = relations[OrderType::LHB][C];
				std::vector<FunctionEvent> DLHB = relations[OrderType::LHB][D];
				if (std::find(CLHB.begin(), CLHB.end(), D) != CLHB.end() && std::find(DLHB.begin(), DLHB.end(), A) != DLHB.end()) {
						error = true;
						errorMessage << "Not FiLo: " << C << " " << COM << " " <<  A << ", " << D << " " << COM << " " << A;
						errorMessage << ", " << C << " " << LHB << " " << D << ", " << A << " " << LHB << " " << B << ", and " << D << " " << LHB << " " << A << "\n";
				}
			}
			if (A.name == B.name && A.name == FunctionName::FN_Push && relations[OrderType::COM][A].size() == 1 && relations[OrderType::COM][B].size() == 1) {
				// A(push) COM C(pop), B(push) COM D(pop).
				FunctionEvent C = relations[OrderType::COM][A][0]; FunctionEvent D = relations[OrderType::COM][B][0];
				std::vector<FunctionEvent> CLHB = relations[OrderType::LHB][C];
				std::vector<FunctionEvent> BLHB = relations[OrderType::LHB][B];
				if (std::find(CLHB.begin(), CLHB.end(), D) != CLHB.end() && std::find(BLHB.begin(), BLHB.end(), C) != BLHB.end()) {
						error = true;
						errorMessage << "Not FiLo: " << A << " " << COM << " " <<  C << ", " << B << " " << COM << " " << D;
						errorMessage << ", " << A << " " << LHB << " " << B << ", " << C << " " << LHB << " " << D << ", and " << B << " " << LHB << " " << C << "\n";
				}
			}
		}
	}
...
/****************************************************************/
/* Checks n properties the hp-queue execution should satisfy. */
/****************************************************************/
void GenMCDriver::checkHp(Event pos) {
	auto &g = getGraph();
	auto *label = g.getEventLabel(pos);
	FunctionEvent currFunEvent = FunctionEvent(label->getMethod(), label->getInvocation());
	// llvm::outs() << "Event: " << g.getReadLabel(pos)<< "\n";
	int funValue;
	bool valExists = false;
	...
	std::string str;
	llvm::raw_string_ostream errorMessage(str);
	errorMessage << "Hp: Found one or multiple modelviolations\n";
	...
	std::map<int, vector<FunctionCall>> groupedThreadFunctionCalls;

    for (auto& f : functioncalls) {
        int value = f.getVal(0);
        groupedThreadFunctionCalls[value].push_back(f);
    }
	...
	for (auto& pair : groupedThreadFunctionCalls) {
        int value = pair.first;
        auto& calls = pair.second;

        // llvm::outs() << "Group with getVal(0) = " << value << "\n";

		std::vector<SVal> addrs;
		for(auto& f : calls) {
			const auto& labels = f.getlabels();
			for(const auto &l : labels) {
			// llvm::outs() << printer.toString(*l) << "\n";
			// llvm::outs() << l->getPos() << "\n";
			// llvm::outs() << l->getPos().thread << "\n";
			// llvm::outs() << l->getPos().index + 1 << "\n";
			auto *lab = g.getEventLabel(Event(l->getPos().thread, l->getPos().index + 1));
			// llvm::outs() << printer.toString(*lab) << "\n";
			auto *labb = g.getReadLabel(Event(l->getPos().thread, l->getPos().index + 1));
			// llvm::outs() << printer.toString(*labb) << "\n";
			// llvm::outs() << getReadValue(labb) << "\n";
			SVal addr = getReadValue(labb);
			addrs.push_back(addr);
			}
		}
		std::set<SVal> addrsUnique(addrs.begin(), addrs.end());
		for(auto& a : addrsUnique) {
			// llvm::outs() << a << "\n";
			int state = 9;
			if (a >= SVal(17)) {
				continue;
			...
			for(auto& f : calls) {
				// llvm::outs() << "Event: " << f.getEvent() << ", Events: "<< "\n";
				// llvm::outs() << state << "\n";
				const auto& labels = f.getlabels();
				auto &l = labels.front();
				auto *labb = g.getReadLabel(Event(l->getPos().thread, l->getPos().index + 1));
				...
				switch (state) {
					...
					default:
						error = true;
						errorMessage << state << "Not covered yet\n";
						break;
				}
				}
				// llvm::outs() << "Updated State: " << state << "\n";
			}
		}
	}
	// llvm::outs() << currFunEvent << "\n";
	// printGraph();
	// llvm::outs() << "####################" << "\n";
...
void GenMCDriver::visitThreadFinish(std::unique_ptr<ThreadFinishLabel> eLab)
{
	...
	if (!isExecutionDrivenByGraph() && /* Make sure that there is not a failed assume... */
		...
		// do not add information here as it is the end of the thread and there is thus no instruction linked to this.
...
void GenMCDriver::visitStore(std::unique_ptr<WriteLabel> wLab, const EventDeps *deps)
{
	...
	/* --add-- */
	// The current label will only ever generate a communication edge when it is part of an interesting functioncall.
	if(g.getPreviousLabel(lab->getPos())->getCOM().size() < lab->getCOM().size()) {
		/* If a COM edge was added then we need to verify if the resulting graph will still be datastructure consistent. */
		checkModelViolations(lab->getPos());
	}
	/* --end-- */
...
{
	if(s == Status::VS_ModelViolation) {
		// TODO find a better way to error report.
		printGraph();
		llvm::outs() << err << "\n";
		halt(s);
	}
...
/* --Add-- */
/* Checks if a functionname is interesting. */
FunctionName GenMCDriver::checkFunctionName(std::string functionName) {
	if (getConf()->datastructureModel.type == DSModelType::M_None) {
		// This methodcall can never be interesting.
		return FunctionName::FN_NON;
	}
	for (auto kv : getConf()->datastructureModel.functioncallMap) {
		if (functionName.compare(kv.second) == 0 && checkNamesForModel(getConf()->datastructureModel.type, kv.first)) {
			return kv.first;
		}
	}
	// Model is implemented but we are not interested in this methodcall.
	return FunctionName::FN_NON;
}

/* Adds a functionstart label to the graph if it is one of the functions we are interested in. Otherwise it returns false. */
void GenMCDriver::visitFunctionStart(std::unique_ptr<FunStartLabel> lab, const EventDeps *deps, FunctionName name) {
	if (isExecutionDrivenByGraph())
		return;
	lab->setMethod(name);
	updateLabelViews(lab.get(), deps);
	auto *eLab = getGraph().addOtherLabelToGraph(std::move(lab));
	//add check here
	// checkModelViolations(eLab->getPos());
}

void GenMCDriver::visitFunctionEnd(std::unique_ptr<FunEndLabel> lab, const EventDeps *deps) {
	if (isExecutionDrivenByGraph())
		return;
	updateLabelViews(lab.get(), deps);
	/* When the functioncall ends we also need to check if everything is still good. */
	// Check if you can only check dequeue here.
	auto *eLab = getGraph().addOtherLabelToGraph(std::move(lab));
	checkModelViolations(eLab->getPos());
}
// --end
			      ...
			      const GenMCDriver::Status &st)
{
	...
	case Status::VS_ModelViolation:
		return s << "Model Violation";
	default:
...
void GenMCDriver::printGraph(bool printMetadata /* false */, llvm::raw_ostream &s /* = llvm::dbgs() */)
{
	...
	for (auto i = 0u; i < g.getNumThreads(); i++) {
		...
		for (auto j = 1u; j < g.getThreadSize(i); j++) {
			...
			// --Added--
			if(lab->getMethod() != FunctionName::FN_NON) {
				s << lab->getMethod();
				s << lab->getInvocation();
				s << "  ";
			}
...
/* --Add-- */
void GenMCDriver::dotPrintCallGraphToFile(const std::string &filename, std::vector<FunctionCall> functions, FunctionEvent error,
					std::map<OrderType, std::map<FunctionEvent, std::vector<FunctionEvent>>> rel)
{
	...
	std::map<int, std::vector<FunctionCall>> fcallPerThread;

	for (auto fcall : functions) {
		int cThread = fcall.getlabels()[0]->getThread();
		fcallPerThread[cThread].push_back(fcall);
	}

	for (auto t : fcallPerThread) {
		ss << "\tsubgraph cluster_" << t.first << " {\n";
		ss << "\t\tlabel=thread"<< t.first << ";\n";
		ss << "\t\t";
		for (auto tc : t.second) {
			// The amount of values this functioncall has.
			int vals = rel[COM][tc.getEvent()].size() + rel[INVCOM][tc.getEvent()].size();
			ss << tc.getEvent();
			if(tc.isDone()) {
				if(vals > 0) {
					ss << "[label=\"" << tc.getEvent() << "(";
					...
					ss << "[label=\"" << tc.getEvent();
					if (tc.getEvent().name == FN_Enq || tc.getEvent().name == FN_Push || tc.getEvent().name == FN_Unlock) {
						ss << "(?)\"";
					} else {
						ss << "(_)\"";
					}
				...
				ss << "[label=\"" << tc.getEvent() << "...\", color=grey";
			}
			if(tc.getEvent() == error) {
				ss << ", color=darkred";
			}
			ss << "]; ";
		}
	...
	for (auto tmap : rel) {
		if(tmap.first == OrderType::INVCOM || tmap.first == OrderType::INVSO) {
			continue;
		}
		for (auto v : tmap.second) {
			if (v.second.size() == 0) {
			...
			for (int i = 0; i < v.second.size(); i++) {
				ss << "\t" << v.first << "->" << v.second[i] << "[label=\"" << tmap.first << "\"";
				if (tmap.first == PO) {
					ss << ", weight=20.0";
				} else if (tmap.first == COM) {
					ss << ", color=red";
				} else if (tmap.first == SO) {
					ss << ", color=green";
				}  else if (tmap.first == LHB) {
					ss << ", color=blue, weight=0.1";
				}
				ss << "]\n";
			}
		}
