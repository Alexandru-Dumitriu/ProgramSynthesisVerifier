...
void Interpreter::visitReturnInst(ReturnInst &I) {
  ...
  // --Add--
  // GV_TO_SVAL(getOperandValue(I.getValOperand(), SF), typ)
  //llvm::outs() << "thread " << I.getFunction()->getName().str() << "\n";
  //if(!I.getFunction()->getName().compare(StringRef("threadW"))) {
	//llvm::outs() << "-- " << I.getFunction()->getName().str() << " + " << I << "\n";
	//llvm::outs() << GV_TO_SVAL(Result, RetTy) <<"\n";
  //}
  // --end
...
void Interpreter::visitCallInstWrapper(CallInstWrapper CS) {
  ...
  // --Add--
  // llvm::outs() << *CS.operator&() << "\n";
  // llvm::outs() << "fun: " << F->getName().str() << "\n";
  // llvm::outs() << CS.getCalledOperand()->getName() << "\n";
  // --end
...
{
	//bool funstarted = false;
	std::map<int, bool> funstarted;
	while (driver->scheduleNext()) {
		...
		/* --add-- */
		if(funstarted.find(getCurThr().id) == funstarted.end()) {
			funstarted[getCurThr().id] = false;
		}

		if (I.hasMetadata("new.function.name")) {
		//	llvm::outs() << " > new function";
			std::string currName = cast<MDString>(I.getMetadata("new.function.name")->getOperand(0))->getString().data();
			std::string::size_type i = currName.find("_start");
			if (i != std::string::npos) {
				currName.erase(i, 6);
				FunctionName name = driver->checkFunctionName(currName);
				if (name == FunctionName::FN_NON) {
					continue;
				}
				auto sdeps = makeEventDeps(nullptr, nullptr, getCtrlDeps(getCurThr().id), getAddrPoDeps(getCurThr().id), nullptr);
				if (funstarted[getCurThr().id]) {
					std::unique_ptr<FunEndLabel> eLab = FunEndLabel::create(nextPos());
					// DEBUG
					WARN_ONCE("abnormal-ret", "DEBUG: This test uses abnormal returns\n");
					driver->visitFunctionEnd(std::move(eLab), &*sdeps);
				}
				std::unique_ptr<FunStartLabel> sLab = FunStartLabel::create(nextPos());
				driver->visitFunctionStart(std::move(sLab), &*sdeps, name);
				funstarted[getCurThr().id] = true;
			} else if (funstarted[getCurThr().id]) {
				auto edeps = makeEventDeps(nullptr, nullptr, getCtrlDeps(getCurThr().id), getAddrPoDeps(getCurThr().id), nullptr);
				std::unique_ptr<FunEndLabel> eLab = FunEndLabel::create(nextPos());
				driver->visitFunctionEnd(std::move(eLab), &*edeps);
				funstarted[getCurThr().id] = false;
			}
		} else {
			// need to skip the unimportant instructions we added.
		//	llvm::outs() << " > visit " << I << " ";
			visit(I);
		}
	//	llvm::outs() << ";\n";
